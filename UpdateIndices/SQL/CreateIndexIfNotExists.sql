...
begin
 ...
 set @TSQLScripCreationIndex='
 IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N''' + QUOTENAME(@SchemaName) + '.' + QUOTENAME(@TableName) + ''') AND name = N''' + @IndexName + ''')
 CREATE '+ @is_unique  +@IndexTypeDesc + ' INDEX ' +QUOTENAME(@IndexName)+' ON ' + QUOTENAME(@SchemaName) +'.'+ QUOTENAME(@TableName)+ '('+@IndexColumns+') '+ 
  ...
  '  
	SET @TSQLScripCreationIndex = REPLACE(@TSQLScripCreationIndex, ', FILLFACTOR =0', '')
